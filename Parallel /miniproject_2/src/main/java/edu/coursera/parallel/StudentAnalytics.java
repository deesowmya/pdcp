...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
        ...
        double average = studentStream.parallel().filter(student -> student.checkIsCurrent() )
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        Stream<Student> studentStream = Arrays.stream(studentArray);
        return studentStream.parallel().filter(student -> !student.checkIsCurrent() )
                .collect(Collectors.groupingBy(Student::getFirstName, Collectors.counting()))
                .entrySet().parallelStream().sorted(Map.Entry.comparingByValue(Comparator.reverseOrder()))
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
        Stream<Student> studentStream = Arrays.stream(studentArray);
        int cnt = (int)studentStream.parallel().filter(s -> (!s.checkIsCurrent() && s.getAge() > 20 && s.getGrade() < 65)  )
                .count();
        return cnt;
    }
}
